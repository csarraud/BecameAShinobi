...
public class KatonWidget extends AbstractSkillWidget implements IUpgradeNatureWidget {

	private int temporaryValue;
	
	public KatonWidget(int x, int y, int width, int height) {
	...
	public List<String> createDescription() {
		...
		if(player.getCapability(CapabilityBecameAShinobi.CAPABILITY_SHINOBI).isPresent()) {
			...
			if(temporaryValue > 0) {
				value = ChatFormatting.GOLD + "" + (data.getChakraNature().getKaton().getValue() + temporaryValue) + " (+" + temporaryValue + ")";
			} 
			else {
				value = ChatFormatting.GOLD + "" + data.getChakraNature().getKaton().getValue();
			}
		}
	...
	public void validateChanges() {
		...
		if(player.getCapability(CapabilityBecameAShinobi.CAPABILITY_SHINOBI).isPresent()) {
			...
			data.getChakraNature().getKaton().addValue(temporaryValue);
