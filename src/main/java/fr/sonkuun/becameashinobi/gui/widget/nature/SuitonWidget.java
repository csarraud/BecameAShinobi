...
public class SuitonWidget extends AbstractSkillWidget implements IUpgradeNatureWidget {

	private int temporaryValue;
	
	public SuitonWidget(int x, int y, int width, int height) {
	...
	public List<String> createDescription() {
		...
		if(player.getCapability(CapabilityBecameAShinobi.CAPABILITY_SHINOBI).isPresent()) {
			...
			if(temporaryValue > 0) {
				value = ChatFormatting.GOLD + "" + (data.getChakraNature().getSuiton().getValue() + temporaryValue) + " (+" + temporaryValue + ")";
			} 
			else {
				value = ChatFormatting.GOLD + "" + data.getChakraNature().getSuiton().getValue();
			}
		}
	...
	public void validateChanges() {
		...
		if(player.getCapability(CapabilityBecameAShinobi.CAPABILITY_SHINOBI).isPresent()) {
			...
			data.getChakraNature().getSuiton().addValue(temporaryValue);
